...
import 'package:jkautomed/models/ajugnu_order_notification.dart';
...
class SupplierNotificationsScreen extends StatefulWidget {
  ...
  const SupplierNotificationsScreen({super.key, required this.onNotificationRead});

  @override
  State<StatefulWidget> createState() {
    return SupplierNotificationsState();
  }
}

class SupplierNotificationsState extends State<SupplierNotificationsScreen> {
  List<AjugnuOrderNotification>? orderNotifications;

  void fetchOrderNotifications() {
    ProductRepository().getOrderNotifications().then((results) {
      widget.onNotificationRead();
      if (results.isNotEmpty) {
        setState(() {
          orderNotifications = results;
          adebug("orderNotification: $orderNotifications");
        });
      } else {
        setState(() {
          orderNotifications = [];
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          // orderNotifications![index].orderId.isNotEmpty?CommonWidgets.text("OrderId - ${orderNotifications![index].orderId}", textColor: Colors.black, fontSize: 15, fontFamily: 'Poly', overflow: TextOverflow.ellipsis, maxLines: 3):SizedBox(),
                          orderNotifications![index].orderId.isNotEmpty?CommonWidgets.text("Product - ${orderNotifications![index].englishName}", textColor: Colors.black, fontSize: 15, fontFamily: 'Poly', overflow: TextOverflow.ellipsis, maxLines: 3):SizedBox(),
                    ...
                    Column(
                      ...
                      children: [
                       ...
                       // if (orderNotifications![index].title.toLowerCase().contains('product sold'))
                      if(orderNotifications![index].orderId.isNotEmpty)
                        Card(
                          color: Colors.black,
                          ...
                          child: InkWell(
                            onTap: () {
                                  ...
                                  orderId: orderNotifications![index].orderId);
